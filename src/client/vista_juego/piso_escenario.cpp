...
PisoEscenario::PisoEscenario(const int render_x, const int render_y, const int render_ancho,
...
void PisoEscenario::dibujar() const {
    ...
    for (int x = render_x; x < render_x + render_ancho; x += ancho_textura_superficie) {
        renderer.Copy(textura_superficie, coords_superficie,
                      SDL2pp::Rect(x, render_y, ancho_textura_superficie, alto_textura_superficie));
    }

    for (int x = render_x; x < render_x + render_ancho; x += ancho_textura_relleno) {
        for (int y = render_y + alto_textura_superficie; y < render_y + render_alto;
