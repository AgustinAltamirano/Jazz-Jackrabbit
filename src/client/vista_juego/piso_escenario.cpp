...
PisoEscenario::PisoEscenario(const SDL2pp::Rect render_coords, SDL2pp::Renderer& renderer,
...
void PisoEscenario::dibujar() const {
    ...
    for (int x = render_coords.GetX(); x < render_coords.GetX() + render_coords.GetW();
        ...
        const SDL2pp::Rect coords_bloque(x, render_coords.GetY(), ancho_textura_superficie,
    ...
    for (int x = render_coords.GetX(); x < render_coords.GetX() + render_coords.GetW();
        ...
        for (int y = render_coords.GetY() + alto_textura_superficie;
             y < render_coords.GetY() + render_coords.GetH(); y += alto_textura_relleno) {
...
PisoEscenario::~PisoEscenario() = default;
