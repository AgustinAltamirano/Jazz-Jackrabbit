...
EnviadorCliente::EnviadorCliente(Socket *skt_cliente, std::atomic<bool> &sigo_en_partida,
                                 std::atomic<bool> &sigo_jugando, GestorPartidas *gestor_partidas, int32_t &id_cliente)
        :
        sigo_en_partida(sigo_en_partida), sigo_jugando(sigo_jugando), cola_enviador(10000),
        recibidor_cliente(skt_cliente, sigo_en_partida, sigo_jugando, id_cliente, cola_enviador),
        gestor_partidas(gestor_partidas),
        servidor_deserializador(skt_cliente), servidor_serializador(skt_cliente), id_cliente(id_cliente) {
...
void EnviadorCliente::run() {
    ...
    while (sigo_jugando) {
        ...
        if (cola_recibidor != nullptr)
            gestor_partidas->borrar_cliente(id_cliente); //Elimino al jugador de la partida en la que estaba
    }
}
...
void EnviadorCliente::cerrar_cola() {
    cola_enviador.close();
}
...
void EnviadorCliente::inicio_recibidor_cliente() {
    ...
    while (sigo_jugando) {
        try {
            ComandoDTO *comando = servidor_deserializador.obtener_comando(&cerrado,
                                                                          id_cliente);
            if (comando->obtener_comando() == CREAR) {
                ...
                cola_recibidor = gestor_partidas->crear_partida(&cola_enviador, id_cliente, codigo_partida,
                                                                capacidad_partida);
                ...
                cola_recibidor = gestor_partidas->unir_partida(&cola_enviador, codigo_partida, id_cliente);
...
bool EnviadorCliente::still_alive() {
    return sigo_jugando;
}
